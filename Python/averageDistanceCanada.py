...
canadaFileName = path + 'canadaData/canadaDivisions.csv'
usFileName = path + 'usData/usDivisions.csv'
region1FileName = path + 'usData/southCanada.txt'
outputFileName = path + 'usData/southCanadaCounties.csv'
fluxFilePath = path + 'fluxUsToContinent/fluxU'
dataFileName = path + 'usData/usMovementDataProcessed.csv'
outputFluxFileName = path + 'usData/southCanadaFluxes.csv'

mexico = loadDivisions(mexicoFileName)
us = loadDivisions(usFileName)
canada = loadDivisions(canadaFileName)

sourceDirectory = us
region1Directory = us
region2Directory = canada
...
for sourceUid in sourceDirectory:
	...
	distToRegion2 = getAverageDistance(sourceUid, sourceDirectory, None, region2Directory)
	maxDist = max(distToRegion1, distToRegion2)
	minDist = min(distToRegion1, distToRegion2)
	if (maxDist/minDist) <= 1.10:
